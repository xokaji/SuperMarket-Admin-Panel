...
import { productData6, productCompanies6 } from '../../../dummyData';
...
const Snacks = () => {
  ...
  const products = Object.keys(productData6);
  ...
  useEffect(() => {
    if (selectedProduct) {
      setSelectedCompany(productCompanies6[selectedProduct][0]);
    }
  }, [selectedProduct]);
  ...
  const handleProductSelect = (event) => {
    ...
    if (product) {
      setSelectedCompany(productCompanies6[product][0]);
    }
  };
  ...
  const filteredData = selectedProduct
    ? productData6[selectedProduct].filter((data) => !selectedMonth || data.month === selectedMonth)
    : [];
  ...
  const generatePDF = () => {
    if (chartRef.current) {
      html2canvas(chartRef.current).then((canvas) => {
        ...
        pdf.save('snacks-stock-report.pdf');
      });
    }
  };
  ...
  return (
    <div className="grocery-container">
      <h1 className="title">Snacks Stock</h1>
      <div className="grocery-viewer">
        <div className="select-container">
          ...
          <select id="product-select" value={selectedProduct} onChange={handleProductSelect}>
            <option value="">Select a Snack Product</option>
        ...
        <div className="select-container">
          ...
          <select id="company-select" value={selectedCompany} onChange={handleCompanySelect}>
            {selectedProduct && productCompanies6[selectedProduct].map((company, index) => (
